...
private fun <T> AbstractTagDecoder.decodeSerializableValuePolymorphic(deserializer: DeserializationStrategy<T>): T {
    if (deserializer !is PolymorphicSerializer<*>) {
        return deserializer.deserialize(this)
    }

    val compound = cast<CompoundTag>(decodeTag())
    val type = compound.getTag(ClassDiscriminator)!!.asString()
    compound.remove(ClassDiscriminator)
    @Suppress("UNCHECKED_CAST")
    val actualSerializer = deserializer.findPolymorphicSerializer(this, type) as KSerializer<T>
    return format.readNbt(compound, actualSerializer)
}

private sealed class AbstractTagDecoder(val format: NbtFormat, open val map: Tag) : NamedValueDecoder() {
