 ...
 * Do you wants these internal functions as part of the API? Please make an issue.
 */
internal fun <T> KSerializer<T>.convertToTag(obj: T) = TagEncoder().also { it.encode(this, obj) }.compoundTag

internal fun <T> KSerializer<T>.fromTag(tag: CompoundTag) = TagDecoder(tag).decode(this)
...
fun <T> KSerializer<T>.put(obj: T, inTag: CompoundTag, key: String? = null) {
    ...
    require(!inTag.containsKey(usedKey)){"""If you are serializing two objects of the same type, you MUST specify a key, see kdoc.
